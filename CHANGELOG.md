...
- TestFlight 1.02 (20) restores expressive Multiplayer identity: waiting-room and live player badges show each selected pet in its half-right pose, waiting rows are taller with more room between the pet and public name, and the assigned color is a trailing game-cell square whose glyph follows Settings. Waiting Room now uses the Multiplayer title treatment, the Multiplayer back control follows the active theme, Pixel-only supporting copy is enlarged on mode, capacity, pet, name, and readiness labels, and Foka/Kesha sit seven points higher over their habitats only in Leaderboard rows.
- TestFlight 1.02 (19) removes pets from the Multiplayer waiting room, gives compact and tall iPhones the same full-size roster/actions with the requested 20-point list offset, and refines live play around a color-dominant Points/color/Lives HUD with an exact 5-point board gap. Two to four smaller player badges now stay in one horizontal row below the Speed Bar with vertically centered pets, single-line score/name/multiplier content, and thicker theme-colored outline glow in every theme.
