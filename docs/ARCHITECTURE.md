...
Use `Debug`, `Staging`, `OwnerAdsQA`, and `Release` configurations with committed `.xcconfig` examples. The first named-cohort Staging configuration is release-optimized but intentionally points to the same P-027 Hostinger compatibility service because no separate native staging backend exists; it must not be mistaken for data isolation. Debug and ordinary Staging use Google demo inventory. The authorized build-6 Staging archive may compare private SHA-256 fingerprints for the owner's cable and TestFlight IDFVs locally to select ignored owner production units plus that phone's GMA test hash; a missing/nonmatching identifier selects demo units. The raw IDFVs are neither stored nor transmitted. Owner Ads QA retains the cable-only path. Checked-in Release is disabled; a separately controlled, explicitly authorized archive supplies live units and no test identifier. Inject only nonsecret identifiers into the app bundle. Private keys and server secrets never ship in the app.
