...
- **Game Center:** explicit optional social surface. `GameCenterService` installs `GKLocalPlayer` authentication only after a Profile opt-in, remembers a successful opt-in, removes its handler when the player chooses Turn Off, and owns only Apple-controller presentation, scoped player identifiers, and ephemeral identity-verification material. Apple exposes no app-level Game Center sign-out API. The selected verified mirror is a future Hostinger-owned server outbox with server-enforced publication consent; the client never submits scores and Game Center is never a prerequisite for local play or another service.
...
Use `Debug`, `Staging`, `OwnerAdsQA`, and `Release` configurations with committed `.xcconfig` examples. The first named-cohort Staging configuration is release-optimized but intentionally points to the same P-027 Hostinger compatibility service because no separate native staging backend exists; it must not be mistaken for data isolation. Debug and ordinary Staging use Google demo inventory. An explicitly accepted owner-split Staging candidate may compare private SHA-256 fingerprints for the owner's cable and TestFlight IDFVs locally to select ignored owner production units plus that phone's GMA test hash; a missing/nonmatching identifier selects demo units. An exact owner production no-fill may make one bounded request with the configured demo fallback. The raw IDFVs are neither stored nor transmitted. Owner Ads QA retains the cable-only path. Checked-in Release is disabled; a separately controlled, explicitly authorized archive supplies live units and no test identifier. Inject only nonsecret identifiers into the app bundle. Private keys and server secrets never ship in the app.
