...
## P-043 — Adopt explicit Apple/Google identity and verified Game Center profile binding

- Date: 2026-07-20
- Status: Accepted for the native client; advances P-005 and the binding prerequisite of P-011/P-042 without enabling Game Center publication

Context: Backend release `hostinger-20260720-1` now verifies Sign in with Apple challenges/tokens/codes, maps Apple and Google identities to one internal player UUID through explicit flows, exposes provider binding state, accepts a fresh signed Game Center `teamPlayerID` proof as a one-to-one secondary link, and revokes Apple authorization during deletion. It deliberately cannot infer that unrelated Apple and Google subjects belong to one human and does not yet hold Game Center publication consent or a score/achievement outbox.

Decision: Add Apple's native authorization control above Google in Profile and enable the Sign in with Apple entitlement. Native provider requests always use explicit `login`, `register`, `link`, or `reauth`; a failed login never auto-registers. Creating a separate profile requires an explanatory confirmation, while retaining an existing wallet requires signing in through its linked method and explicitly linking the second provider. Pass the backend's raw Apple nonce/state unchanged, request no name/email scopes, retain credentials only for the active exchange, validate the returned state and expected internal UUID, and never merge by email or nickname. Account deletion may reauthenticate through either linked primary provider and still requires the exact destructive phrase.

Keep Game Center optional and link-only. After explicit Connect/Verify, request a server challenge, obtain a fresh GameKit signature, and send only the exact signed `teamPlayerID` proof fields. Treat `identityBindings.gameCenter` as evidence that the profile has a binding, not evidence that the currently authenticated Game Center player owns it; require an explicit runtime Verify round-trip before naming that player as verified. A conflict never rebinds automatically. Turn Off cancels local participation but cannot claim server unlink because no unlink route exists.

Consequences: Existing Google UUIDs, wallets, results, purchases, cosmetics, and achievements remain intact while players can add Apple deliberately. Game Center still cannot create/access a wallet or authorize gameplay/economy routes. No direct `GKLeaderboard`/`GKAchievement` submission is added, and Profile states plainly that scores and achievements are not published. Physical signed-device validation remains required for Apple authorization presentation, Apple credential revocation/deletion, Google↔Apple linking conflicts, Game Center account changes, parental restrictions, cancellation, and relaunch. The server still needs publication consent/unlink semantics, an environment-aware idempotent outbox, and correction policy before P-011/P-042 mirroring can activate.

Revisit when: the provider API gains stable machine error codes, the compatibility cookie/CSRF contract is replaced by mobile-v1, Game Center publication support ships, or Apple changes native authorization/GameKit identity semantics.

## P-044 — Diagnose and bound beta ad recovery while AdMob readiness is pending

- Date: 2026-07-20
- Status: Accepted implementation candidate for version `1.02` build `8`; upload remains gated on observed creative

Context: The AdMob console reports PimPoPom as **Requires review**, with no linked public App Store details. Its seven-day app overview recorded 36 production requests, zero impressions, and a 0.00% match rate. AdMob cannot find the unpublished App Store Connect record, and production inventory therefore remains blocked on a public listing, store linkage, and review. Exact source comparison shows that builds 6 and 7 have byte-identical ad services, configuration code, tests, and RootView bootstrap wiring; Sign in with Apple did not introduce an ad-code regression. The archive already carried Google's official demo units as the non-owner route, yet the runtime never used them after an owner production no-fill. A transient UMP update failure could also leave one process permanently failed because the unchanged ads-allowed session did not restart eligibility.

Physical and Simulator diagnostics then established a separate unresolved condition: authenticated and anonymous sessions both resolved ads as allowed, UMP returned `canRequestAds == true`, and GMA reached the network, but the official demo banner/interstitial returned exact no-fill. Repeating the request with Google's public sample App ID and demo units produced the same response. Google documents those demo units as account-independent test inventory, so the production readiness block cannot explain that result. The public Google Ads status page reported no declared AdMob incident at the time; treat this as an unresolved upstream delivery anomaly rather than a proven platform outage.

Decision: Prepare, but do not upload, the named Staging candidate `1.02` build `8` until a real official demo creative renders during the final network pass. Preserve authoritative server `adFree` gating and UMP `canRequestAds`; neither may be bypassed. On the registered owner route only, try the configured production banner/interstitial first. If and only if GMA returns its exact no-fill domain/code, make one bounded retry with the corresponding official Google demo unit. Non-owner devices continue to request the demo units directly. Configurations whose primary and fallback IDs match cannot retry, and checked-in Release remains disabled. Do not retry invalid-request, network, timeout, server, or other failures as a demo fallback.

After a UMP refresh fails while no stored consent permits ads, make one explicit post-bootstrap retry and one retry on each later foreground transition while the state remains failed and the authoritative account remains ads-allowed. Never retry a valid consent-blocked state, run timers, recurse, or reload merely because SwiftUI reattached a banner host. Log only route labels, consent/account classifications, and public SDK error domain/code/message; never log IDFV values, their hashes, test-device identifiers, cookies, or account tokens.

Consequences: The owner route cannot silently fail after a production no-fill, UMP transport failure is recoverable without bypassing consent, and diagnostics distinguish account, consent, routing, and public GMA error classes without identifiers. These safeguards do not manufacture inventory and are not evidence that ads work. A new launch starts with the production owner route again, so AdMob readiness can be observed after the app becomes publicly listed and approved. An actual demo creative is a build-8 upload gate; App Store publication/linking, AdMob review, `app-ads.txt`, consent-region coverage, ad-free purchase validation, interstitial presentation, and accidental-tap review remain production gates.

Revisit when: an official demo creative renders again, AdMob reports the app Ready, a public App Store listing can be linked, production units achieve a nonzero match rate, the owner IDFV changes, or this named TestFlight exception is removed.

## P-045 — Commit closed-beta AdMob identifiers and expose explicit launch diagnostics

- Date: 2026-07-20
- Status: Accepted for the closed internal beta; refines P-037/P-038/P-044 without lifting the build-8 upload gate

Context: Optional ignored AdMob files made the installed route difficult to audit and let stale owner fingerprints silently choose demo inventory. A separate decoder regression introduced with build 5 also discarded `PimPoPomAdMobTestDeviceIDs` in `owner-real-test`, causing Owner Ads QA to disable itself before UMP or GMA while printing only the valid `ads-allowed` account state. The owner explicitly authorized keeping the nonsecret AdMob identifiers and closed-beta device selectors in tracked configuration and printing them during deliberate cable diagnostics.

Physical diagnostics after repairing that decoder used App ID `ca-app-pub-6428992187280935~3622035442`, production banner `ca-app-pub-6428992187280935/3513535878`, production interstitial `ca-app-pub-6428992187280935/5433122203`, and programmatic GMA test-device ID `7269f20fc2c3dde327884d9c4903745b`. UMP returned `canRequestAds == true`, but both production requests returned exact no-fill with valid Google response IDs and no mediation line items. The same iPhone then requested Google's official fixed banner and interstitial demo units with an empty custom test-device list over both Wi-Fi and cellular; both networks produced the same valid-response-ID/no-line-item result. Exact build-4 source reproduced demo no-fill as well. This rules out PHP login/ad-free state, owner routing, the Apple-login changes, unit/App ID transcription, UMP eligibility, the custom GMA test hash, and the local network. `app-ads.txt`, App Store linkage, and AdMob readiness remain relevant to production units but cannot explain account-independent official demo-unit failure.

The earlier `7269…` identifier had served production Test-mode ads during prior physical QA. On 2026-07-20, however, Ad Inspector run against the exact `com.otcsoftware.pimpopom` bundle and production AdMob App ID reported `65889f215752fbc9ad39e52b00d92987` as the device's current GMA test identifier. This value is distinct from the UMP debug identifier and is used for subsequent owner-device production-unit diagnostics.

Decision: Commit the public PimPoPom AdMob App ID, both production unit IDs, both official demo unit IDs, the registered GMA test-device ID, and the accepted owner IDFV SHA-256 fingerprints in the named `.xcconfig` files. Remove the ignored AdMob include files. Debug always uses official demo units with no custom test-device ID. Staging uses the production/Test-mode route only when its local IDFV fingerprint matches; every other device uses demo units with no custom test identifier. Owner Ads QA always uses production units plus the registered programmatic test-device ID. Fix the decoder so Owner Ads QA retains that ID and so forbidden identifiers remain visible to validation in disabled/demo/live modes. When an owner production no-fill triggers the one bounded official demo fallback, retain the registered test-device configuration so any later production-unit request in the same process cannot lose Test mode; official demo units remain test inventory regardless.

Permit `--ad-diagnostics` cable launches to print the raw app-visible IDFV, its SHA-256 fingerprint, route/account/UMP state, public GMA error details, response IDs, and mediation response JSON. Normal launches print none of these diagnostics. Continue to exclude cookies, CSRF values, identity tokens, StoreKit JWS, account tokens, private keys, and server secrets. Keep version `1.02` build `8` unuploaded until an actual demo creative renders; response IDs establish delivery to Google but do not satisfy that gate.

Consequences: Every internal archive's AdMob route can be reconstructed from Git plus the phone's launch diagnostics, the cable-only production path no longer disables itself, and demo fallback remains bounded without making a later production request unsafe. The owner fingerprints and GMA test-device ID are now repository-visible closed-beta metadata by explicit product-owner choice. This does not create inventory or bypass AdMob review. The remaining demo no-fill is upstream of PimPoPom's request integration and should be rechecked with Ad Inspector/Google support using the retained response IDs.

Revisit when: Google restores official demo creative delivery, the current owner IDFV or GMA test-device identifier changes, the closed cohort expands, build 8 is replaced, or public Release ads are authorized.

## P-046 — Bootstrap UMP from app launch instead of identity restoration

- Date: 2026-07-20
- Status: Accepted for version `1.02` build `8`; supersedes P-033/P-044 only for consent-bootstrap sequencing

Context: PimPoPom restored the PHP session before requesting UMP information and only entered the consent flow after the backend resolved an ad-supported account. A fresh EEA install could therefore reach login without receiving the required launch consent form, and identity changes were incorrectly coupled to privacy-message presentation. The account authority and consent authority are independent: UMP determines whether Google may receive an ad request, while the backend determines whether this PimPoPom profile is entitled to an ad-free experience.

Decision: On every ads-enabled app launch, request updated UMP consent information and load/present any required form before restoring Google, Apple, or PHP identity. Cache that result for the process. Login, logout, provider linking, and account switching only reconcile the cached consent result with authoritative `sessionState.adFree`; they must never trigger a second UMP request or form. Keep GMA unconfigured and unstarted while the account is unresolved or ad-free. Start it only when UMP reports `canRequestAds` and the account resolves ads-allowed. Await GMA's initialization result and serialize duplicate session notifications so no banner or interstitial request can overtake SDK initialization. If the launch refresh fails without stored permission, allow the existing single post-bootstrap retry after account resolution and one retry on each later foreground transition; never retry a valid blocked state automatically. A transition to ad-free still destroys inventory and clears cadence.

Add Debug-only `--ump-debug-eea` and `--ump-debug-reset` launch arguments so the named iPhone SE (2022) Simulator can reproduce a first-install EEA form deterministically. The reset hook is compiled out of Staging and Release and must never run in production.
...
Decision: Install `GKLocalPlayer.local.authenticateHandler` at app launch and let Apple's standard system UI handle authentication when required. This remains nonblocking: cancellation, parental controls, Apple sign-out, network failure, or Game Center service failure cannot block local play, primary PimPoPom login, PHP ranking, shops, purchases, or deletion. Do not persist a local participation preference.

Whenever both an authenticated Apple/Google PimPoPom session and an authenticated GameKit player with persistent scoped IDs are available, silently reconcile that exact profile/player context. Refresh the PHP session, request a one-use challenge, obtain fresh GameKit identity-verification material, and submit the current `teamPlayerID`, persistent `gamePlayerID`, proof tuple, and `publish: true`. Deduplicate an unchanged successful context in-process, reset on profile or Game Center player change, and retry a deferred failure on a later foreground/session transition without creating a request loop. The iOS client does not present or locally arbitrate ownership conflicts.

Reduce the Profile card to a **Game Center** label and repeatable **See stats** button. Remove Connect, Verify Player, Disable Game Center, publication-state descriptions, conflict/reset/operator copy, and local opt-out controls. Opening Apple statistics remains delegate-managed so it works repeatedly. PHP remains the sole score and achievement publisher; iOS still never calls `GKLeaderboard.submitScore` or `GKAchievement.report`, never sends client-authored progress, and never grants wallet/profile authority from Game Center.

Adopt a separate backend **current-profile-wins** reassignment policy so a valid current PHP session plus fresh GameKit proof can move the active scoped pair from an old internal profile. That work must retain session/CSRF, challenge, signature, certificate, timestamp, replay, encryption, uniqueness, publication-lock, and outbox authority; it must not merge profiles or move paid/game data. The copy-ready task is [`GAME_CENTER_AUTOLINK_PHP_TASK.md`](GAME_CENTER_AUTOLINK_PHP_TASK.md). Until it is deployed, an existing `403`/`409` remains a silently deferred Game Center side effect rather than a gameplay failure. Apple may retain scores or achievements already published to the old scoped account because its supported server API does not provide per-player erasure.

Keep Arcade **Your Color** empty during ticket preparation and the one-second Get Ready interval. Reveal only the color chosen by the engine's actual run start; Zen retains its intentional **Any** treatment.

Consequences: Game Center follows the Apple player already active on the device, Profile becomes a simple statistics gateway, and transient Apple/PHP failures stay out of the core game. Automatic reconciliation still cannot make the client authoritative, infer primary-account ownership, or guarantee immediate Apple propagation. The backend must deploy the reassignment transaction before an old cross-profile binding can move, and physical TestFlight validation must cover first-launch authentication, signed-out play, later primary login, same-context deduplication, current-player change, reassignment, dashboard reopening, one verified Arcade best, achievement backfill, and Apple propagation. The pre-run presentation requires deterministic UI coverage proving no stale Arcade color appears before `start`.
...
- Date: 2026-07-29
- Status: Accepted and shipped to TestFlight `1.2 (15)`; matching PHP proof-v3 support is deployed in Hostinger release `20260729-1`
...
## P-057 — Add peer-consistent 2–4-player Multiplayer over GameKit

- Date: 2026-07-29
- Status: Accepted implementation candidate; automated integration, physical multi-device TestFlight validation, and release remain pending

Context: Hostinger backend release `20260729-1` now supplies authenticated Multiplayer lobbies, immutable manifests, bounded transcript replay, settlement, immutable result rows, and a dedicated Game Center publication lane. Reaction-critical taps and HUD updates cannot tolerate an HTTP round trip, while accepting a peer's aggregate score would violate the existing protocol-verification boundary. The backend deliberately does not provide a websocket or live relay.

Decision: Add **Multiplayer** below Zen for 2–4 primary-authenticated profiles with confirmed public nicknames, publishing-enabled Game Center links, and Game Center proof refreshed within ten minutes for create/join/roster/start mutations. Multiplayer is own-color only and awards no coins or achievements. PHP owns lobby membership, stable seats/colors, the immutable `multiplayer-own-color-v1` manifest, final replay, settlement, and ranked rows. `GKMatch` owns reliable reaction-critical packets; no tap, target, decoy, pet, crown, tone, or live HUD change calls PHP.

Bridge the private positive 31-bit PHP `playerGroup` into `GKMatchRequest.playerGroup`, connect exactly the PHP lobby's participant count, and elect the lexicographically smallest persistent `gamePlayerID` as a fixed coordinator. Every device submits the identical complete GameKit roster/coordinator to PHP before the creator may start. The coordinator authors future activation plans and the canonical compact integer event stream. It queues local and remote inputs, sorts them by `(inputAt, seat, inputSequence)`, commits only inputs at or before `coordinatorNow - 250 ms`, and advances logical time only to that watermark. `handledAt` remains handler-lag evidence and never moves the canonical clock past the reorder boundary.

Replicate versioned reliable envelopes containing match ID, packet sequence, event sequence, and logical milliseconds. Broadcast each player's input evidence to every peer, freeze the verified Game Center sender-to-participant/seat/color map before start, and refuse peer-consistent submission when a committed input event lacks matching sender evidence or accepted evidence remains unexplained. Future activation plans are presentation hints rather than transcript events and can be cancelled without creating sequence gaps. Use acknowledgements plus transcript/snapshot checkpoints—including pending plans and the shared clock anchor—for gap and reconnect recovery. A coordinator-authored pause freezes logical time for a bounded reconnect and resumes by shifting the common start anchor; it never advances deadlines while inputs are blocked. Protocol v1 keeps one coordinator for the match and does not migrate authority after a post-start disconnect. If the complete stream cannot be recovered within the bounded reconnect window, cancel/forfeit rather than inventing or accepting divergent evidence.

Every participant submits the same server manifest hash and exact transcript. PHP derives score, rating, multiplier, dodge, life loss, placement, and leaderboard fields. Live iOS HUD values are derived locally from that same event stream and are provisional until settlement. Rank score descending, hits descending, average reaction ascending, then seat ascending. Describe clean accepted results as **protocol-verified, peer-consistent**. Matching peers reduce unilateral coordinator rewriting but do not prevent colluding or modified clients from manufacturing a matching structurally valid transcript; never call this mode server-authoritative, human-verified, bot-proof, or collusion-proof.

Consequences: The UI needs an availability gate, lobby browser/create capacity, waiting room/readiness, GameKit matchmaking/roster confirmation, live 2–4-player strip with pets/colors/points/multiplier/crown, recovery state, settlement/result rankings, and a Multiplayer leaderboard. Another player's accepted hit advances the shared tone sequence on every device. PHP and the client retain the exact 2,500-event/15-minute bounds. Unit/Simulator tests can validate schemas, replay, ordering, clocks, packet loss/gaps, and UI states, but only physical TestFlight devices/accounts can validate real GameKit matchmaking, latency, reconnect behavior, audio synchronization, and unanimous submission.
...
## P-061 — Preserve terminal Multiplayer evidence and compact the live presentation

- Date: 2026-07-30
- Status: Accepted for TestFlight `1.02 (18)`; physical multi-device validation remains required

Context: Two physical players could complete the same match yet both remain at **Checking every player** with zero accepted submissions. The local transport preflight incorrectly treated the `handledAt` member of hit/miss tuples as their ordering time, while the shared protocol and PHP validator order every tuple by member 2—`inputAt` for hits and misses. A terminal miss is immediately followed by `playerOut` at that same input time, so any handler lag made the otherwise valid transcript look nonmonotonic and prevented the HTTP submission entirely. Separately, a coordinator could commit a terminal remote miss at a handler time beyond its reorder watermark, then attempt to advance the already-finished engine backward to the watermark before broadcasting the terminal batch.

Decision: Validate chronological transcript order using tuple member 2 for every event type and retain `handledAt` only as handler-lag evidence. Once a coordinator input finishes the engine, do not advance again; broadcast the complete miss, player-out, and finish batch at a logical time no earlier than the engine clock. Retain each device's exact manifest hash and transcript across transient submission failures and process termination, retry only the same payload, scope every asynchronous callback to that match, poll settlement without inventing local acceptance, ignore stale nonterminal callbacks after a terminal response, and keep Menu unavailable until PHP returns settled, review, or cancelled.

Replace the Multiplayer hub and waiting-room promotional heading with the PimPoPom wordmark plus an MP suffix, remove the ordinary roster-loading card, and stack waiting-room participants. Reuse the single-player Speed Bar without a pet. Present Points, assigned-color cell, and Lives in one compact HUD; omit Top Score, Survived, and the Your Color caption. Stack same-height player badges below the Speed Bar with pet at left, score/name centered, pet-scale multiplier at right, and the leader crown over the top-right corner. Tighten the HUD-to-board spacing and four-player waiting-room geometry on compact 667-point devices while retaining the larger layout elsewhere.
