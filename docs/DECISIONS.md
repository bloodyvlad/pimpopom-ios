...
Replicate versioned reliable envelopes containing match ID, packet sequence, event sequence, and logical milliseconds. Future activation plans are presentation hints rather than transcript events and can be cancelled without creating sequence gaps. Use acknowledgements plus transcript/snapshot checkpoints for gap and reconnect recovery. Protocol v1 keeps one coordinator for the match; it does not migrate authority after a post-start disconnect. If the complete stream cannot be recovered within the bounded reconnect window, cancel/forfeit rather than inventing or accepting divergent evidence.
