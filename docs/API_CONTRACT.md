...
The iOS client keeps Game Center authentication independent of primary PimPoPom authentication and defaults participation to off. It installs `GKLocalPlayer.local.authenticateHandler` only after a Profile **Connect** action, remembers only a successful opt-in, and removes the handler when the player chooses **Turn Off**. Successful Game Center authentication alone cannot log in, register, access a wallet, or authorize any economy/profile route.

For an authenticated PimPoPom profile, an explicit **Link Profile** or **Verify** action first obtains `/api/profile/game-center/challenge`, then obtains a fresh GameKit identity-verification tuple and submits the exact signed `teamPlayerID`, HTTPS public-key URL, standard padded Base64 signature/salt, and integer millisecond timestamp. The server validates Apple's certificate chain, challenge/timestamp freshness, replay, and a one-to-one binding. The app never sends `gamePlayerID` as proof. A profile-level `identityBindings.gameCenter` flag does not prove that the currently authenticated Game Center player is the bound player; each Profile presentation describes it as an existing link until an explicit runtime Verify proof succeeds. A different current Game Center account receives a conflict and is never rebound silently.

The proof is memory-only. **Turn Off** cancels local participation and outstanding client work but is not Apple account sign-out and does not remove the server binding because no unlink route exists. GameKit exposes no app-level sign-out operation, so the Apple account remains managed in iOS Settings. The binding still has no server-held publication consent, plaintext scoped ID usable by an outbox, score outbox, or achievement outbox; therefore the current client truthfully publishes nothing to Game Center.

The selected board remains one future server-fed mirror of each player's protocol-verified Arcade all-time best. The iOS client has no Game Center score-submission call. Before enabling the mirror, the backend-owning repository still needs server-held publication consent/unlink semantics, a retained Apple-scoped identifier suitable for the supported server API, an idempotent outbox, prerelease/production routing, and correction policy. A device-local preference alone is not sufficient authorization for a server outbox. The App Store Connect API credential belongs only on the server and is not a Game Center client-authentication secret.

The same future binding and consent may project the five authoritative PimPoPom achievements through Apple's server-side player-achievement submission API. Use an explicit allowlist rather than constructing vendor IDs from arbitrary response data:
...
The current backend exposes binary locked/claimable/claimed state, not trustworthy numeric progress. Report only monotonic 100% completion when an item first becomes `claimable` or is already `claimed`; never publish 0% or treat the separate coin-reward claim as the unlock. After an explicit binding, reconcile historical server eligibility before backfill and enqueue later unlocks transactionally. TestFlight submissions use Apple's prerelease flag; production performs an environment-correct backfill. Turn Off revokes publication consent and cancels unsent work but does not erase Apple history. Account deletion removes binding, consent, and pending work without calling Apple's reset-all-achievements API.
