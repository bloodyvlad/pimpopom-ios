...
The real AdMob App ID is present in every configuration so UMP can resolve PimPoPom's dashboard message. Debug and ordinary named Staging use Google demo units. The explicitly authorized build-5 Staging archive stores only a SHA-256 fingerprint of the owner's IDFV and selects ignored production units plus the GMA test-device hash on that matching phone; all nonmatching devices retain demo units. The raw IDFV is neither committed nor transmitted by PimPoPom, and a changed/missing IDFV fails safely to demo inventory. Owner Ads QA keeps the separate cable path, and every production-unit owner creative must visibly say **Test mode** before any interaction. Checked-in Release is disabled; live units are an explicit controlled-archive input and cannot coexist with a test hash. The committed 50-entry `SKAdNetworkItems` snapshot was reviewed against Google's official iOS setup list on 2026-07-19 and must be refreshed whenever the pinned GMA version or Google's list changes.
