...
- Resolve every directional companion from horizontal distance to the visible sprite center; vertical tap distance is irrelevant. Alignment within 0.5 native point is front, distance through 15% of the active interaction width on either side is a persistent half-left/right pose, and farther taps are full left/right. Use screen width in menu/gameplay and the 80-point shop preview width. Baseline turn duration is 300 ms and wake/settle transition is 450 ms. Non-game pets settle after 5 seconds; gameplay companions stay awake.
