...
- **Unit-tested:** 29 pure Swift checks cover scoring/rounding, phase/grid boundaries, lives/recovery, proof timing/order, deadline equality, multiplier overflow/reset/5× accounting, decoy overlap/expiry/reservation/ignored opportunities, reset behavior, and Zen retention/cadence/manual results. Sixty-nine native app tests additionally cover cosmetics and API contracts, audio lifecycle, preferences, gameplay presentation, and server-authoritative Achievements. Current additions lock exact ranked build `20260718-1`, start/finish ticket metadata, forward-compatible session/finish decoding with an unknown `achievementSnapshot`, compiled name metadata, opaque Disco corner-base ordering, and the bounded rounded-halo geometry.
- **Simulator-tested:** all 22 XCUITest paths pass on the named iPhone SE (3rd generation, 2022) Simulator with iOS 26.5 at implementation commit `bab0709`. Xcode reported 91 passed tests—69 native unit and 22 UI—with zero failures, zero skips, and zero expected failures; including the separate core package, the full check exercised 120 tests. Current paths retain the Leaderboard/gameplay/onboarding/Achievements coverage and exercise the revised Disco gameplay surface. Fixtures are local and prevent production mutations. The earlier 15 device-independent paths passed independently on iPhone 13 mini and iPhone 13 Pro at implementation commit `ec71d21`; those profiles were not rerun for the current batch.
- **Simulator-inspected:** exact implementation commit `bab0709` was directly inspected through the **SE Disco gameplay polish** XCUITest attachment. The active cyan cell remained saturated and scratched; its light followed a narrow rounded contour instead of a square frame, while all four outer corner wedges remained black over the concrete/reflected-light field. The capture remains local Xcode-result evidence and was not added to the repository. Earlier exact-commit evidence remains recorded below, including three-profile captures at `ec71d21`.
...
- **Not yet validated:** structured physical visual/listening/touch review of implementation commit `bab0709`, measured 60/120 Hz touch timing, a real authenticated score or achievement claim using the `20260718-1` gate, signed-in shop/profile/economy mutations, current-batch 13 mini/13 Pro layout, Game Center integration, accessibility matrix, audio balance/routes/interruptions/Silent switch, haptics, live ads, and StoreKit.
...
The current exact-commit evidence for implementation commit `bab07090ccea2a42f34d2ebfc4a176d9bf3b3ef1`, plus retained iOS 26.5 screenshots, hashes, configurations, and limitations for earlier commits, is recorded in [`DESIGN_QA.md`](DESIGN_QA.md).
