...
- **Not yet validated:** physical review of implementation commit `b9038fe`, measured 60/120 Hz touch timing, a real authenticated score or achievement claim using the `20260719-1` gate, signed-in shop/profile/account-deletion mutations, StoreKit Sandbox/TestFlight purchase/pending/restore/refund/reversal/Family Sharing on an iPhone, server-notification delivery/reconciliation, current-batch 13 mini/13 Pro layout, live Game Center authentication and server account binding, the disabled server-fed Game Center mirror, accessibility/Reduce Motion matrix, audio balance/routes/interruptions/Silent switch, haptics, and live ads.
...
The current automated exact-commit evidence is for implementation commit `b9038fe`, while retained iOS 26.5 screenshots, hashes, configurations, and limitations for earlier visual checkpoints are recorded in [`DESIGN_QA.md`](DESIGN_QA.md).
