...
- **Unit-tested:** 29 pure Swift checks cover scoring/rounding, phase/grid boundaries, lives/recovery, proof timing/order, deadline equality, multiplier overflow/reset/5× accounting, decoy overlap/expiry/reservation/ignored opportunities, reset behavior, and Zen retention/cadence/manual results. Thirty-three native app tests additionally cover cosmetics and API contracts, audio lifecycle, persisted Glyphs/motivation state, the exact 26-slogan pool and 10-second rotation, Pixel/Light tokens, pet geometry and 30-degree facing thresholds, SpriteKit glyph removal, response-progress drain/clamping, and StoreKit-placeholder safety.
- **Simulator-tested:** all 16 XCUITest paths pass on the named iPhone SE (3rd generation, 2022) Simulator with iOS 26.5. The 15 device-independent paths also pass independently on the named iPhone 13 mini and iPhone 13 Pro profiles; only the assertion for exact SE menu geometry is excluded there. Coverage includes Arcade/Zen, fixed non-scrolling menu geometry, near-full-width gameplay, response-bar drain, gameplay pet placement, glyph hiding, menu-pet sleep/wake, slogan tap rotation, Theme/Pet shops, select/hide/show, settings, Buy Coins, and Remove Ads. The fixtures are local and prevent production network mutations.
- **Simulator-inspected:** retained captures from implementation commit `ec71d21` show the Light fixed menu on SE and 13 mini, the transparent Light gameplay board and bottom ad reservation on SE, and the larger Pixel typography/fixed menu on 13 Pro. The Light gameplay shell has no black SpriteKit corners or decorative capsule bars. Parent web commits `923a38e`, `7582b2d`, and `209ee6c` are the menu/theme, pet, and latest-slogan references respectively.
...
- **Asset-tested:** 56 retained runtime/master/source/licence files pass committed SHA-256 checks; runtime audio format/duration, pet sheet dimensions, Disco texture dimensions/copies, Jersey 10 registration/copy, and absence of Pancake bitmap art from the app target are validated.
...
- **Not yet validated:** structured physical visual/listening/touch review of implementation commit `d3ffd87`, measured 60/120 Hz touch timing, real Google iOS OAuth and signed-in shop/ranked mutations, accessibility matrix, audio balance/routes/interruptions/Silent switch, haptics, live ads, and StoreKit.
...
The retained iOS 26.5 screenshots, hashes, configurations, and limitations for implementation commit `ec71d2169c45c7b1f3ca4d7f0434aa48d36a0314`, together with earlier checkpoints, are recorded in [`DESIGN_QA.md`](DESIGN_QA.md).
