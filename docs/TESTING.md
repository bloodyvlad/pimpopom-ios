...
- **Unit-tested:** 29 pure Swift checks cover scoring/rounding, phase/grid boundaries, lives/recovery, proof timing/order, deadline equality, multiplier overflow/reset/5× accounting, decoy overlap/expiry/reservation/ignored opportunities, reset behavior, and Zen retention/cadence/manual results. Seventy-five native app tests additionally cover cosmetics and API contracts, audio lifecycle, preferences, gameplay presentation, and server-authoritative Achievements. Current additions lock the absence of a Disco square underlay, 22/15/11-point radii, shared y-up/y-down board geometry, transparent-center halo alpha/cache/layers, deadline target/halo synchronization, protocol-valid gap misses and life-loss audio, authoritative all-rating feedback payloads, `Good - 802ms`, `+2,343 points`, 15-point score origin, captured Points/Speed Bar destinations, clamped direct motion with decreasing destination distance, Foka-only mirrored right poses, the five-point glowing Your Color frame, horizontal logo-gradient Zen tokens, red HUD color, the eight-point footer lift, 50-point banner host, and preserved compact-device board width.
- **Simulator-tested:** all 22 XCUITest paths pass on the named iPhone SE (3rd generation, 2022) Simulator with iOS 26.5 at implementation commit `6743fc2`. Xcode reported 96 passed tests—74 native unit and 22 UI—with zero failures, zero skips, and zero expected failures; including the separate core package, the full check exercised 125 tests. Current paths retain the Leaderboard/gameplay/onboarding/Achievements coverage and exercise Disco, gap handling, transparent feedback, Zen HUD semantics, the raised pet/Speed Bar footer, 50-point banner, and a 351-point pet-enabled board. Fixtures are local and prevent production mutations. The earlier 15 device-independent paths passed independently on iPhone 13 mini and iPhone 13 Pro at implementation commit `ec71d21`; those profiles were not rerun for the current batch.
- **Simulator-inspected:** exact implementation commit `6743fc2` was directly inspected through focused SE XCUITest attachments. Disco showed matching shell/cell curves, continuous concrete corners, no square ghost, and an outgoing halo above the lit cell. Arcade showed a transparent `Godlike - 200ms` stamp traveling toward the Speed Bar while the grouped score remained above the tap; a Good capture stayed at its border lane. Zen showed the normal-size radial-rainbow Any cell, red infinity, Speed Bar copy, and the standard banner host. The captures remain local Xcode-result evidence and were not added to the repository. Earlier exact-commit evidence remains recorded below, including three-profile captures at `ec71d21`.
- **Build-tested:** the app builds for a generic iOS Simulator with Swift 6 strict concurrency and the resolved Google Sign-In dependency; compiled metadata is asserted to use display/bundle name **PimPoPom**, identifier `com.otcsoftware.pimpopom`, the registered primary/alternate icons, and the Change Icon shortcut.
...
- **Not yet validated:** physical installation or structured visual/listening/touch review of implementation commit `6743fc2`, measured 60/120 Hz touch timing, a real authenticated score or achievement claim using the `20260718-1` gate, signed-in shop/profile/economy mutations, current-batch 13 mini/13 Pro layout, Game Center integration, accessibility/Reduce Motion matrix, audio balance/routes/interruptions/Silent switch, haptics, live ads, and StoreKit.
...
The current exact-commit evidence for implementation commit `6743fc2763b7cae85587f7ffe22f8be75922bc4b`, plus retained iOS 26.5 screenshots, hashes, configurations, and limitations for earlier commits, is recorded in [`DESIGN_QA.md`](DESIGN_QA.md).
...
- Zen's normal-size radial-rainbow Any cell plus red infinity, and Arcade's red hearts, retain non-color accessibility labels;
