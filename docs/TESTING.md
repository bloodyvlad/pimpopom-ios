...
- UI automation injects `FakeConsentService` and `FakeAdsService`; it never contacts Google. It verifies exact banner geometry, both responsive Remove Ads placements, ad-free/disabled absence, pushed-screen ownership, the eligible active-run reservation, and the accessible required Privacy choices action.
