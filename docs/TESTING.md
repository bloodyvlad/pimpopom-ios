...
- **Unit-tested:** 29 pure Swift checks cover scoring/rounding, phase/grid boundaries, lives/recovery, proof timing/order, deadline equality, multiplier overflow/reset/5× accounting, decoy overlap/expiry/reservation/ignored opportunities, reset behavior, and Zen retention/cadence/manual results. Eighty-three native app tests additionally cover cosmetics and API contracts, audio lifecycle, preferences, gameplay presentation, server-authoritative Achievements, non-blocking Game Center authentication, automated-test isolation, player-change rejection, and runtime identity-signature validation. Current additions lock the absence of a Disco square underlay, 22/15/11-point radii, shared y-up/y-down board geometry, transparent-center halo alpha/cache/layers, deadline target/halo synchronization, protocol-valid gap misses and life-loss audio, authoritative all-rating feedback payloads, `Good - 802ms`, `+2,343 points`, 15-point score origin, captured Points/Speed Bar destinations, clamped direct motion with decreasing destination distance, Foka-only mirrored right poses, the five-point glowing Your Color frame, horizontal logo-gradient Zen tokens, red HUD color, the eight-point footer lift, 50-point banner host, and preserved compact-device board width.
- **Simulator-tested:** all 22 XCUITest paths pass on the named iPhone SE (3rd generation, 2022) Simulator with iOS 26.5 at implementation commit `97e8a67`. Xcode reported 105 passed tests—83 native unit and 22 UI—with zero failures, zero skips, and zero expected failures; including the separate core package, the full check exercised 134 tests. Current paths retain the Leaderboard/gameplay/onboarding/Achievements coverage and exercise Disco, gap handling, transparent feedback, direct Points/Speed Bar absorption, Zen HUD semantics, the raised pet/Speed Bar footer, 50-point banner, and a 351-point pet-enabled board. Fixtures are local and prevent production mutations. The earlier 15 device-independent paths passed independently on iPhone 13 mini and iPhone 13 Pro at implementation commit `ec71d21`; those profiles were not rerun for the current batch.
...
- **Not yet validated:** physical installation or structured visual/listening/touch review of implementation commit `97e8a67`, measured 60/120 Hz touch timing, a real authenticated score or achievement claim using the `20260718-1` gate, signed-in shop/profile/economy mutations, current-batch 13 mini/13 Pro layout, live Game Center authentication and server account binding, the disabled server-fed Game Center mirror, accessibility/Reduce Motion matrix, audio balance/routes/interruptions/Silent switch, haptics, live ads, and StoreKit.
...
The current automated exact-commit evidence is for implementation commit `97e8a67`, while retained iOS 26.5 screenshots, hashes, configurations, and limitations for earlier visual checkpoints are recorded in [`DESIGN_QA.md`](DESIGN_QA.md).
