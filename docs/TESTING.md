...
- **Simulator-tested:** all 25 XCUITest paths pass on the named iPhone SE (3rd generation, 2022) Simulator with iOS 26.5 at build-3 preparation commit `eb1cd09`. Xcode reported 138 passed tests—113 native unit and 25 UI—with zero failures, zero skips, and zero expected failures; including the separate core package, the full check exercised 167 tests. Current paths include the localized StoreKit entry points, binding gates, four coin-product review captures, the balance-free Remove Ads review capture, Restore Purchases, Profile ordering, exact account-deletion confirmation, and existing Leaderboard/gameplay/onboarding/Achievements, Game Center isolation, Disco, gap handling, Zen, pet-follow, audio-control, Theme Shop, and compact-layout coverage. Fixtures are local and prevent production mutations. The earlier 15 device-independent paths passed independently on iPhone 13 mini and iPhone 13 Pro at implementation commit `ec71d21`; those profiles were not rerun for the current batch.
...
- **Not yet validated:** physical review of build-3 preparation commit `eb1cd09`, measured 60/120 Hz touch timing, a real authenticated score or achievement claim using the `20260719-1` gate, signed-in shop/profile/account-deletion mutations, StoreKit Sandbox/TestFlight purchase/pending/restore/refund/reversal/Family Sharing on an iPhone, server-notification delivery/reconciliation, current-batch 13 mini/13 Pro layout, live Game Center authentication and server account binding, the disabled server-fed Game Center mirror, accessibility/Reduce Motion matrix, audio balance/routes/interruptions/Silent switch, haptics, and live ads.
...
The current automated exact-commit evidence is for build-3 preparation commit `eb1cd09`, while retained iOS 26.5 screenshots, hashes, configurations, and limitations for visual checkpoints are recorded in [`DESIGN_QA.md`](DESIGN_QA.md).
