...
- **Unit-tested:** 29 pure Swift checks cover scoring/rounding, phase/grid boundaries, lives/recovery, proof timing/order, deadline equality, multiplier overflow/reset/5× accounting, decoy overlap/expiry/reservation/ignored opportunities, reset behavior, and Zen retention/cadence/manual results. Sixty-one native app tests additionally cover cosmetics and API contracts, audio lifecycle, preferences, gameplay presentation, and the server-authoritative Achievements slice: exact GET/claim routes, CSRF/body, catalog decoding/validation, stale identity, expired session/CSRF recovery, malformed-success rejection, controller claim/refresh serialization, exact fallback IDs/rewards, solid cross fill, vivid Disco tokens and black-corner layering, and 32 unique Pixel grain squares with a 24-bright/8-dark split.
- **Simulator-tested:** all 19 XCUITest paths pass on the named iPhone SE (3rd generation, 2022) Simulator with iOS 26.5 at implementation commit `fd11703`. The Xcode result reports 80 passed tests, zero failures, zero skips, and zero expected failures. The added offline achievement path verifies one ready and one claimed state across all five cards, claims 1 coin from a 9-coin balance, advances progress from 1/5 to 2/5, and updates the menu summary. The fixtures are local and prevent production network mutations. The earlier 15 device-independent paths passed independently on iPhone 13 mini and iPhone 13 Pro at implementation commit `ec71d21`; those profiles were not rerun for the current batch.
- **Simulator-inspected:** implementation commit `fd11703` was directly inspected on the SE profile for the Achievements screen in Classic, Light, Disco, and Pixel; a strongly saturated active Disco target with opaque black rounded-corner surround; brighter clipped square Pixel grain in live gameplay; and solid cross glyphs in Classic, Light, and Disco Theme Shop previews. No new screenshot was retained. Earlier exact-commit evidence remains recorded below, including three-profile captures at `ec71d21`. Parent web commits `923a38e`, `7582b2d`, and `209ee6c` remain the menu/theme, pet, and latest PHP visual references respectively.
...
- **Not yet validated:** physical install or structured physical visual/listening/touch review of implementation commit `fd11703`, measured 60/120 Hz touch timing, real Google iOS OAuth, a real authenticated achievement claim, signed-in shop/ranked mutations, current-batch 13 mini/13 Pro layout, Game Center integration, accessibility matrix, audio balance/routes/interruptions/Silent switch, haptics, live ads, and StoreKit.
...
The current no-new-capture evidence for implementation commit `fd11703731f0cec0a160323729439cd96a365f4a`, plus retained iOS 26.5 screenshots, hashes, configurations, and limitations for earlier commits, is recorded in [`DESIGN_QA.md`](DESIGN_QA.md).
