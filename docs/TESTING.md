...
- **Unit-tested:** 29 pure Swift checks cover scoring/rounding, phase/grid boundaries, lives/recovery, proof timing/order, deadline equality, multiplier overflow/reset/5× accounting, decoy overlap/expiry/reservation/ignored opportunities, reset behavior, and Zen retention/cadence/manual results. Twenty-six native app tests cover catalog/action matrices, selected/hidden/equipped/special-pet resolution, current pet asset dimensions and presentation geometry, buy/select/hide/show API mutations, audio manifests and exact resource formats, independent persisted audio preferences, exact music-transition generation/context matching, gameplay terminal lifecycle routing, response-progress drain/clamping, API/session/Google/CSRF contracts, stale-session rejection, serialized economy mutations, and theme contrast.
- **Simulator-tested:** nine XCUITest paths pass on the named iPhone SE (3rd generation, 2022) Simulator with iOS 26.5. Coverage includes Arcade, Zen manual results, Theme Shop/Buy Coins, Pet Shop/placeholder policy/Buy Coins, independent Music/Sound FX settings, static-until-tapped pet previews, special Muse presentation on menu/gameplay, decreasing active-round response progress, and Select → Hide → Show pet state changes. The fixtures are local and prevent production network mutations.
- **Simulator-inspected:** native Zen target and complete bottom layout fit the iPhone 13 mini and iPhone 13 Pro simulator profiles. Retained SE captures show Muse independently positioned on the main menu without an in-flow icon above the mode controls, Pet Shop sprites resting on their habitats while idle, and Muse plus a decreasing response bar during Arcade. Parent web repository commit `7582b2d` is the pet-presentation parity reference.
...
- **Asset-tested:** 47 retained runtime/master/source files pass committed SHA-256 checks; runtime audio format/duration and current pet sheet dimensions are validated, and no Pancake bitmap or retained master enters the app target.
...
- **Not yet validated:** physical-device install/launch/listening/touch review of implementation commit `3b022c3`, measured 60/120 Hz touch timing, real Google iOS OAuth and signed-in shop/ranked mutations, accessibility matrix, audio balance/routes/interruptions/Silent switch, haptics, live ads, and StoreKit.
...
The retained iOS 26.5 screenshots, hashes, configurations, and limitations for implementation commit `3b022c3287d41213159ecdcda39437b18e0b1c35`, together with earlier checkpoints, are recorded in [`DESIGN_QA.md`](DESIGN_QA.md).
