...
Build `1.02 (7)` from archived source commit `2012d1e` is processed as VALID in TestFlight, available to Internal QA, and waiting for Apple Beta App Review for External QA. It adds native Sign in with Apple, deliberate Apple/Google account registration and linking, recent-provider reauthentication for deletion, and verified opt-in Game Center profile binding. The signed archive and embedded provisioning profile contain the `com.apple.developer.applesignin` Default entitlement plus Game Center. Build `1.01 (6)` remains historical TestFlight evidence. The current 13 mini/13 Pro matrix, Build 7 TestFlight installation, live Apple login/link/reauthentication, verified Game Center binding, Sandbox purchase/restore, consent/revocation, production-unit creative, three-final interstitial, accidental-tap review, and 60/120 Hz timing still require physical validation.
