# Current design QA evidence

This file keeps only evidence relevant to the current TestFlight presentation.
Earlier checkpoints and screenshot paths remain in Git history. Evidence here does
not override `CURRENT_VERSION.md` or prove physical-device behavior.

## TestFlight 1.02 (20) — 2026-07-30

- Exact archive source: `69fe7422719dd4953e90354a2ae3f3c976995db7`.
- Named Simulator: PimPoPom iPhone 17.
- Focused Cosmetics and Multiplayer presentation tests passed 47/47.
- Waiting-room fixtures passed in Classic, Disco, Light, and Pixel. Captures were
  inspected for selected half-right pets, square assigned-color cells, glyph on/off,
  public-name/readiness separation, Pixel small-copy scale, and compact four-player
  fit.
- Four-player live fixtures passed in all themes. Captures were inspected for pet
  pose, assigned-color outline/glow, score/name/multiplier, crown, and one-row fit.
- The Pixel Leaderboard marketing fixture passed with the accepted Foka/Kesha
  habitat alignment.
- Waiting Room uses the Multiplayer title treatment; its back control is theme
  styled and lowered. Build 20 supersedes build 19's pet-free waiting contract.

After the final six-point waiting-name spacing edit, the owner skipped the remaining
Simulator rerun and full repository gate before archive. The archive compiled and
processed, but the final spacing is not backed by a post-edit capture in this record.

## Still required

- Physical iPhone review of build 20 on compact/tall 60 Hz and ProMotion layouts.
- Real 2-, 3-, and 4-player waiting/live/results states with distinct accounts.
- Long/localized names, largest supported text, VoiceOver, bold text, Increase
  Contrast, Reduce Motion, and glyph-off review in every theme.
- Dynamic network/reconnect/settlement states and FAST predicted/reconciled states.
- Final App Store screenshots from the submitted production-facing build. The
  retained launch kit uses older build-6 captures and is source/provenance only.

For a new visual checkpoint record exact commit/build, Simulator/device/OS,
configuration, scenario, theme, accessibility settings, artifact path/hash, what
was inspected, result, and remaining limitations.
