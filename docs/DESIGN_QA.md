...
Build `1.02 (10)` from archived source commit `68ad4d25e83ec98f4127e848c53543be1294f23e` is the latest TestFlight upload and is approved/in beta testing for both Internal QA and External QA. Its exact source passed 29 deterministic core checks plus all 213 native/UI paths on the named iPhone SE 2022 Simulator with zero failures, skips, or expected failures. The changes are service/lifecycle and Profile-state corrections rather than a new visual direction: Game Center is gated behind primary login, retained authentication reconnects without waiting indefinitely, conflict copy/actions are unambiguous, and enabled audio has deterministic foreground-resume state. Physical TestFlight validation remains required, especially for the actual conflicting accounts, PHP outbox/Apple propagation, audible app-switch recovery, the current 13 mini/13 Pro matrix, a successful Google creative, Sandbox purchase/restore, consent revocation, three-final interstitial, accidental-tap review, and 60/120 Hz timing.

## TestFlight 1.02 corrective checkpoint — 2026-07-25

- **Candidate/version/build:** PimPoPom 1.02 (10), named Staging/TestFlight release; bundle `com.otcsoftware.pimpopom`, visible name `PimPoPom`, minimum iOS 17.0.
- **Archived source:** exact source commit `68ad4d25e83ec98f4127e848c53543be1294f23e` (`Prepare TestFlight 1.02 build 10`) on `codex/testflight-1-02-build-10`.
- **Automated checks:** `Scripts/check.sh` passed project generation, strict formatting, asset/provenance and privacy/configuration guards, the generic Swift 6 Simulator build, 29 deterministic core tests, and all 213 native/unit/UI paths on the named iPhone SE (3rd generation) Simulator, for 242 passing checks with zero failures, skips, or expected failures. Result bundle: `/Users/vlad/Library/Developer/Xcode/DerivedData/PimPoPom-hejlughidecerddzjroxolkasjul/Logs/Test/Test-PimPoPom-2026.07.25_22-25-39-+0200.xcresult`.
- **Corrective behavior:** Apple/Google link outcomes now remain scoped to the intended internal profile and appear beside their controls; provider ownership conflicts never silently merge accounts. Game Center is unavailable before primary PimPoPom authentication, already-authenticated GameKit sessions reconnect without waiting for another callback, Connecting can be cancelled immediately, and a conflicting link can be turned off locally without modifying its owning profile. Enabled menu/gameplay audio resumes after ordinary background/foreground transitions while non-resumable interruptions and route loss retain a trusted-action gate.
- **Identity limitation:** an Apple identity or Game Center `teamPlayerID` already linked to another PimPoPom UUID remains a real server conflict. The client neither steals nor merges that identity. PHP can enqueue score and achievement backfill only after Game Center is successfully linked to the exact owning primary profile, or after a separately audited backend identity transfer/reset.
- **Archive/upload:** retained archive `/Users/vlad/Documents/PimPoPom-release-artifacts/1.02-10/PimPoPom-1.02-10-68ad4d2.xcarchive` (43 MB), deterministic sorted-file manifest SHA-256 `9487ac2c5d9f43e0dd698177df1c405e652c0bafa2d7d8b1c4a69342eb0bcfa2`, and exported IPA `/Users/vlad/Documents/PimPoPom-release-artifacts/1.02-10/upload/PimPoPom.ipa` with SHA-256 `f61c09b5c90b36ff813983abd4c432eed5e3135ff1df67daa04361810d2186da`. The distribution-signed app passed App Store validation with Apple Sign In and Game Center entitlements, no `get-task-allow`, no private key or local StoreKit catalog, and the intended Staging owner-split ad configuration.
- **TestFlight state:** App Store Connect accepted build ID `0e6f59d9-0cf2-4282-8f94-9d2ddbab2ca0` as `VALID`, with the icon present and `usesNonExemptEncryption = false`. Internal QA and External QA both report `IN_BETA_TESTING`, automatic notification is enabled, and Beta App Review is `APPROVED`.
- **Production/device scope:** archive, upload, review, and TestFlight availability are verified. A physical TestFlight run must still confirm the actual Apple/Google ownership-conflict path, a successful Game Center link/backfill using the owning profile, later Apple leaderboard/achievement propagation, Turn Off/cancel/reconnect, and audible menu/gameplay recovery after app switching.

## TestFlight 1.02 Game Center publication checkpoint — 2026-07-25

- **Candidate/version/build:** PimPoPom 1.02 (9), named Staging/TestFlight candidate; bundle `com.otcsoftware.pimpopom`, visible name `PimPoPom`, minimum iOS 17.0.
- **Archived source:** exact source commit `5be2f05b09d171cb169ab15eba1f034f2a9aedf0` (`Prepare TestFlight 1.02 build 9`) on `codex/testflight-1-02-build-9`.
- **Automated checks:** `Scripts/check.sh` passed project generation, strict formatting, asset/provenance and privacy/configuration guards, the generic Swift 6 Simulator build, 29 deterministic core tests, and all 204 native/unit/UI paths on the named iPhone SE (3rd generation) Simulator, for 233 passing checks with zero failures, skips, or expected failures. Focused backend, cosmetic, and Game Center suites separately passed 78 tests. Result bundle: `/Users/vlad/Library/Developer/Xcode/DerivedData/PimPoPom-hejlughidecerddzjroxolkasjul/Logs/Test/Test-PimPoPom-2026.07.25_19-49-46-+0200.xcresult`.
- **Game Center contract:** the app obtains the PHP challenge before asking GameKit for the signed proof, submits persistent `teamPlayerID` plus `gamePlayerID` with explicit publication consent, models linked/queued/ready/held/conflict/reset-needed states, and disables future publication through the server while retaining the verified identity link. The client contains no `GKLeaderboard.submitScore` or `GKAchievement.report` path; PHP remains authoritative and publishes only protocol-verified Arcade personal bests and eligible achievement unlocks.
- **Live compatibility probe:** immediately before archive, the deployed signed-out session reported `serverPublicationAvailable: true`, `preReleased: true`, no linked identity/publication, zero pending or held jobs, and no reset requirement. This matches the additive build-9 decoder and does not expose or prove the exact backend deployment commit.
- **Archive/upload:** retained archive `/Users/vlad/Documents/PimPoPom-release-artifacts/1.02-9/PimPoPom-1.02-9-5be2f05.xcarchive` (43 MB); deterministic sorted-file manifest SHA-256 `b2735985b1ed7b94912e5bddef6f39d8fc8c9e52f65e3155effcaf24da60fd64`. Its archived app passed strict signature inspection, carries Apple Sign In and Game Center entitlements, version `1.02 (9)`, the Staging owner-split ad mode, and no `.p8` key or local StoreKit test catalog. App Store Connect accepted build ID `aa47097b-9ab5-45ec-85d0-096ea645e7f3` as `VALID`, with `usesNonExemptEncryption = false` and the app icon present.
- **TestFlight state:** assigned to Internal QA and External QA with automatic notification enabled. App Store Connect reports both groups `IN_BETA_TESTING`; the beta submission is `APPROVED`. The English beta description, per-build What to Test, and reviewer notes cover optional Apple/Google sign-in, server-fed Game Center score/achievement mirroring and Turn Off, Sandbox StoreKit, AdMob/UMP, Privacy Choices, and account deletion without requiring reviewer credentials.
- **Production/device scope:** upload and TestFlight availability are verified, but build 9 has not yet been installed from TestFlight or used to exercise a physical Game Center challenge/link/backfill, later Arcade best delivery, five achievement deliveries, Turn Off, conflict, held/reset-needed recovery, Apple propagation delay, real Sandbox purchase/restore, or successful Google creative.
