...
Build `1.02 (7)` from archived source commit `2012d1e` remains the latest TestFlight upload and is approved/in beta testing for both Internal QA and External QA. Build-9 candidate source has passed 29 deterministic core checks plus all 204 native/UI paths on the named iPhone SE 2022 Simulator, including the exact supplied Pixel star, tall-layout/multiplier refinements, deterministic screenshot fixture, and server-authoritative Game Center linking/publication contracts. It is not described as a TestFlight build until the clean committed Staging archive is accepted by Apple. Local build `1.02 (8)` at implementation commit `01854fb` remains an unuploaded historical AdMob diagnostic candidate. Physical Game Center link/backfill/publication, the current 13 mini/13 Pro matrix, a successful Google creative, Sandbox purchase/restore, consent revocation, three-final interstitial, accidental-tap review, and 60/120 Hz timing still require validation.
