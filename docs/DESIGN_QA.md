...
Build `1.02 (9)` from archived source commit `5be2f05b09d171cb169ab15eba1f034f2a9aedf0` is the latest TestFlight upload and is approved/in beta testing for both Internal QA and External QA. Its exact source passed 29 deterministic core checks plus all 204 native/UI paths on the named iPhone SE 2022 Simulator, including the exact supplied Pixel star, tall-layout/multiplier refinements, deterministic screenshot fixture, and server-authoritative Game Center linking/publication contracts. Local build `1.02 (8)` at implementation commit `01854fb` remains an unuploaded historical AdMob diagnostic candidate. Physical Game Center link/backfill/publication, the current 13 mini/13 Pro matrix, a successful Google creative, Sandbox purchase/restore, consent revocation, three-final interstitial, accidental-tap review, and 60/120 Hz timing still require validation.

Build `1.02 (10)` is an unreleased corrective candidate. Its full gate passed 29 deterministic core checks plus 213 native/UI paths on the named iPhone SE 2022 Simulator with zero failures, skips, or expected failures. The changes are service/lifecycle and Profile-state corrections rather than a new visual direction: Game Center is gated behind primary login, retained authentication reconnects without waiting indefinitely, conflict copy/actions are unambiguous, and enabled audio has deterministic foreground-resume state. Physical TestFlight validation remains required, especially for the actual conflicting accounts, PHP outbox/Apple propagation, and audible app-switch recovery.
