...
- The current PHP client contract was most recently audited at legacy repository release commit `0330524be797066369f0cac14e2b13cee219b44f` on 2026-07-19. Live HTML, JavaScript, health, and signed-out session probes confirmed Hostinger Season 1, deployed build ID `20260719-1`, and the new wallet/ad-free/StoreKit response shape, matching retained annotated deployment tag `hostinger-20260719-1`; this still does not prove an unrecorded future deployment.
...
- Ads/StoreKit: ads remain a disabled placeholder, and the reserved 50-point banner host remains below the lifted pet/Speed Bar footer without shrinking the near-full-width board. Lower-right Remove Ads, Theme Shop Buy Coins, and Pet Shop Buy Coins share the real StoreKit 2 sheet. It loads localized App Store products, requires a signed-in server-bound PimPoPom profile, submits only the verified JWS and server-issued `appAccountToken`, waits for authoritative PHP credit before finishing, recovers unfinished transactions from launch/foreground, restores only the non-consumable, and displays authoritative wallet, refund debt, and ad-free state. A Debug-only local scheme uses an offline fake credit service; no physical Sandbox/TestFlight purchase has been accepted as validated yet.
