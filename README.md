...
- Visual parity: the fixed, non-scrolling native main menu, wordmark, illuminated intro/slogan treatments, Arcade/Zen controls, feature hierarchy, backgrounds, two-column Theme Shop, detailed Leaderboard/Profile/results surfaces, custom gameplay header/HUD, near-full-width board, border reaction stamps, and Speed streak/multiplier translate the reviewed web design into SwiftUI and SpriteKit. The web coin art and black-bordered coin/rank badges are shared across native surfaces; the menu trophy shows the signed-in player's Arcade position. Classic, Light, and Pixel effects are code-native; Pixel uses the reviewed Jersey 10 font at a native 10% scale increase and Disco uses the exact reviewed texture set. Light uses a clean sky gradient, a readability plate behind its logo, no capsule “cloud” artifacts, and a transparent SpriteKit scene that exposes the white rounded board shell. The Achievements entry is present but its catalog/claim sheet remains an explicit placeholder.
- Themes and pets: all four theme palettes plus reviewed current Foka/Kesha/Tauta/Misha/Mitsuri sprite sheets, server-derived Muse special-pet art, and the owner-approved native Pancake replacement with a glowing blue floor are native resources. A theme is selected or bought by tapping its whole tile; Disco previews keep their black base independently of the active theme. Placement is surface-specific and shop previews stay static until tapped. Pet facing ignores vertical distance: near-exact horizontal alignment is front, the first 15% of the interaction width on either side is half-turned, and farther taps are fully turned. Menu pets sleep after inactivity, the active menu pet is shifted left, and the gameplay pet sits at 40% of screen width. Pancake uses the normal 500-coin server purchase path; the client does not invent ownership or price.
...
- Simulator regression evidence: implementation commit `cccdae2` passed 29 core tests, 40 native unit tests, and all 17 iPhone SE 2022 XCUITest paths on iOS 26.5. Direct SE inspection covered the revised menu utilities, Theme Shop, live Leaderboard, signed-out Profile/Game Center placeholder, centered gameplay glyph, clean Game Over, and Pet Shop placement. The earlier 13 mini/13 Pro regression remains recorded at commit `ec71d21` and was not rerun for this batch. The current checkpoint was not installed or tested on physical hardware.
