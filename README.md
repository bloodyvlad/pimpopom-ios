...
- The current PHP client contract and visual behavior were most recently audited at legacy repository `main` commit `371d59815fa7c9e562e69d05962ac6063de0b40f` on 2026-07-18. Live HTML, JavaScript, and health probes confirmed Hostinger Season 1 and deployed build ID `20260718-1`, matching retained annotated deployment tag `hostinger-20260718-1` at that commit; this still does not prove an unrecorded future deployment.
...
The app currently uses SwiftUI, SpriteKit, `PimPoPomCore`, `URLSession`, Google Sign-In for iOS, GameKit, and one app-owned `AVAudioEngine` service. Ads and StoreKit remain disabled placeholders; Google activates only when a real iOS OAuth client ID is supplied through the ignored local configuration.
...
- Ads/StoreKit: disabled placeholders. The reserved 50-point banner host remains below the lifted pet/Speed Bar footer without shrinking the near-full-width board. Lower-right Remove Ads, Theme Shop Buy Coins, and Pet Shop Buy Coins each open an explicit StoreKit placeholder that grants no value or entitlement.
...
- Simulator regression evidence: implementation commit `6743fc2` passed 29 core checks, 74 native unit tests, and all 22 XCUITest paths on the named iPhone SE 2022 Simulator with iOS 26.5; Xcode reported 96 passed tests and zero failures or skips (125 checks including the core package). Compiled metadata is asserted as display/bundle name **PimPoPom** and bundle identifier `com.otcsoftware.pimpopom`. Direct SE inspection confirmed matching rounded Disco shell/cell curves with no rectangular ghost, outgoing active-cell light, transparent fading rating stamps, grouped score flyouts, Godlike absorption into the Speed Bar, the Zen rainbow/red HUD treatment, the 50-point banner host, and a retained 351-point board with a pet present. Contract checks cover protocol-valid gap misses, the exact deployed `20260718-1` ranked-start gate, and forward-compatible decoding of the additive `achievementSnapshot` field. The earlier 13 mini/13 Pro regression remains recorded at commit `ec71d21` and was not rerun for this batch. No authenticated Hostinger write was sent.
- App Store Connect record `PimPoPom` and explicit App ID `com.otcsoftware.pimpopom` exist under team `APX2925X66`; the first TestFlight upload/configuration is tracked under P-027. A backend-native API, final logo/launch-cue acceptance, StoreKit products, and ad account are not complete.
